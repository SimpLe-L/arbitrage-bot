...
// Pangolin Factory and Router addresses on AVAX
pub const PANGOLIN_FACTORY: &str = "0xefa94DE7a4656D787667C749f7E1223D71E9FD88";
pub const PANGOLIN_ROUTER: &str = "0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106";

// Event signatures (similar to UniswapV2)
...
pub async fn pangolin_related_contract_addresses() -> Vec<String> {
    vec![
        PANGOLIN_FACTORY.to_string(),
        PANGOLIN_ROUTER.to_string(),
    ]
}

#[derive(Debug, Clone)]
pub struct PangolinPoolCreated {
    pub token0: Address,
    pub token1: Address,
    pub pair: Address,
...
impl TryFrom<&Log> for PangolinPoolCreated {
...
impl PangolinPoolCreated {
    pub async fn to_pool(&self) -> Result<Pool> {
        ...
        Ok(Pool {
            protocol: Protocol::Pangolin,
            pool: self.pair,
            tokens,
            extra: PoolExtra::Pangolin { fee_rate: 300 }, // 0.3% fee for Pangolin
        })
    }
}

#[derive(Debug, Clone)]
pub struct PangolinSwapEvent {
    pub sender: Address,
    pub amount0_in: U256,
    pub amount1_in: U256,
...
impl TryFrom<&Log> for PangolinSwapEvent {
    type Error = eyre::Error;
...
pub fn is_pair_created_event(log: &Log) -> bool {
    !log.topics.is_empty() && 
    log.address.to_string().to_lowercase() == PANGOLIN_FACTORY.to_lowercase() &&
