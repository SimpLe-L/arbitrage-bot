...
// 实现从其他错误类型的转换
impl From<ethers::providers::ProviderError> for BotError {
    fn from(err: ethers::providers::ProviderError) -> Self {
        BotError::RpcError(err.to_string())
    }
}

impl From<ethers::abi::ParseError> for BotError {
    fn from(err: ethers::abi::ParseError) -> Self {
        BotError::Configuration(format!("Parse error: {}", err))
    }
}

impl From<std::num::ParseIntError> for BotError {
    fn from(err: std::num::ParseIntError) -> Self {
        BotError::ConfigError(format!("Parse int error: {}", err))
    }
}

impl From<ethers::utils::ConversionError> for BotError {
    fn from(err: ethers::utils::ConversionError) -> Self {
        BotError::Configuration(format!("Conversion error: {}", err))
    }
}
