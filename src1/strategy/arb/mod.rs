...
use crate::core::collectors::{Event, EventHandler};
...
impl EventHandler for ArbitrageHandler {
    async fn handle_event(&mut self, event: Event) -> Result<()> {
        match event {
            ...
            Event::NewBlock { block_number, block_hash, timestamp } => {
                // 可以在新区块时更新池状态等
                log::debug!("收到新区块 #{}: {}", block_number, block_hash);
                Ok(())
            }
            Event::System(system_event) => {
                // 处理系统事件
                match system_event {
                    crate::core::collectors::SystemEvent::Connected => {
                        log::info!("套利处理器收到连接事件");
                    }
                    crate::core::collectors::SystemEvent::Disconnected => {
                        log::warn!("套利处理器收到断开连接事件");
                    }
                    crate::core::collectors::SystemEvent::Error(error) => {
                        log::error!("套利处理器收到错误事件: {}", error);
                    }
                    crate::core::collectors::SystemEvent::Shutdown => {
                        log::info!("套利处理器收到关闭事件");
                    }
                }
                Ok(())
            }
